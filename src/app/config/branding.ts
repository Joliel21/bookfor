...
const DEFAULT_BRANDING: BrandingConfig = {
  ...
  logoUrl: "https://raw.githubusercontent.com/Joliel21/rare_revolution_magazine_2026/main/assets/images/logos/magazine-source/public/brand/rare-revolution-trademark-logo.png",
...
export const loadBranding = async (): Promise<BrandingConfig> => {
  const configuredUrl =
    typeof window !== "undefined"
      ...
      : undefined;
  const url = configuredUrl || `${import.meta.env.BASE_URL}branding.json`;

  try {
    const response = await fetch(url, { cache: "no-store" });
    if (!response.ok) throw new Error(`Branding returned ${response.status}`);
    activeBranding = mergeBranding(await response.json());
  } catch (error) {
